...
class Evaluator:
    def __init__(self, name: str, datasets: list, get_predictors: callable, output_dir: {str}=None,
        ...
        self.predictor_names = [x.__class__.__name__ for x in self.get_predictors(1)]
    ...
    def measure_pipeline_run_results(self, y_true, y_pred):
        ...
        self.logger.debug(classification_report(y_true, y_pred, target_names=target_names))
        self.logger.debug(f'MCC={mcc}, Accuracy={acc}, Precision={prec}, Recall={rec}, F1={f1}')
        return {
            'prec': prec,
            'rec': rec,
            'f1': f1,
    ...
    def __call__(self):
        ...
        predictions = pd.DataFrame(0, index=range(0, 6000), columns=multiindex)
